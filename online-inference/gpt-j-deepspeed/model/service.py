...
class Model(kserve.Model):
    ...
    def load_config(self):
        ...
        self.config = AutoConfig.from_pretrained(options['MODEL_PATH'], local_files_only=True)
        if options['MODEL_CONFIG'] != '':
            cfg = json.loads(options['MODEL_CONFIG'])
            for ck, cv in cfg.items():
                logger.info(f'{ck}:{cv}')
                if hasattr(self.config, ck):
                    logger.info(f'Config.{ck} changed from {getattr(self.config, ck)} to {cv}')
    ...
    def load_tokenizer(self):
        ...
        if os.path.isfile(os.path.join(tokenizer_path, 'tokenizer.txt')):
            with open(os.path.join(tokenizer_path, 'tokenizer.txt'), 'r') as file:
                tokenizer_path = os.path.join(tokenizer_path, 'tokenizer', file.read().replace('\n', ''))
    ...
    def load(self):
        ...
        logger.info(f'Loading model from {options["MODEL_PATH"]} into device {options["MODEL_DEVICE"]}:{torch.cuda.get_device_name(options["MODEL_DEVICE"])}')
        ...
        logger.info(f'Model is ready in {str(time.perf_counter() - start_time)} seconds.')
        self.ready = True
    ...
    def warmup(self):
        ...
        logger.info(f'Device {options["MODEL_DEVICE"]}:{torch.cuda.get_device_name(options["MODEL_DEVICE"])}')
        for i in range(1, options['BENCHMARK_WARMUP_ROUNDS']):
            ...
            input_ids = self.tokenizer(text, return_tensors="pt").input_ids.long().cuda()
            min_len = seq_len + 40
    ...
    def benchmark(self):
        ...
        logger.info(f'Device {options["MODEL_DEVICE"]}:{torch.cuda.get_device_name(options["MODEL_DEVICE"])}')
        logger.info('{:<10} {:<10} {:<10} {:<10}'.format('Batch size', 'Seq len', 'Max len', 'time'))
        ...
        for batch_size in range(1, max_batch_size + 1):
            ...
            for seq_len in range(128, 2049, 128):
                ...
                logger.info('{:>10} {:<10} {:<10} {:<10}'.format(batch_size, seq_len, max_length,
                                                                 round(benchmark_time / options['BENCHMARK_SEQUENCE_ROUNDS'], 2)))
        logger.info(f'Total benchmark time {round(time.perf_counter() - benchmark_start_time, 2)}s')
